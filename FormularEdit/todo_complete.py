...
class MainWin(QMainWindow, Ui_MainWindow):
    ...
    def delete(self):
        ...
        self.save()
        self.statusBar.showMessage(u'Deleted {} items!'.format(count), 0)
    ...
    def clicked(self,index):
        ...
        for i in self.model.todos:
            if i[0]:
                len_indexes += 1
        self.statusBar.showMessage(u'Selected {} items!'.format(len_indexes), 0)

    def load(self):
        with open(r"./output/data.json", "r") as f:
            self.model.todos = json.load(f)
        if not len(self.model.todos):
            self.statusBar.showMessage(u'No historical records are currently saved!', 0)
